...
Section~\ref{ssec:m-surrogate} \citep{Coraci2025StageCalibration}: Stage~A/B/C
...
Eq.~\eqref{eq:m-ms} \citep{Wang2025SafetyMetric}) are defined once in
Section~\ref{ssec:m-eval} and not repeated here. The transferability study follows
a pre-registered, Popperian discipline: each hypothesis (H1--H4) is bound to a
...
Transfer succeeds at the level of the calibration \emph{pipeline} but not the frozen
\emph{policy}. Re-running the Stage~A/B/C inverse calibration on the hydronic family
reduces rollout \RMSE{} by $60.2$--$87.8\%$, and the zone capacitance re-identifies
as a near-uniform structural invariant ($\Czon$ scales by $1.918\pm0.032\times$ the
source value), so the physical parameterisation is portable in a precise,
quantified sense. Zero-shot transfer of the frozen controller, by contrast, is
regime-dependent and either fails or passes only with an energy penalty. This is a
sharper, component-level statement than the usual ``transfer reduces training
cost'' claim \citep{Hou2024MultiSource}: it identifies \emph{what} transfers (the
identified physics) and \emph{what} does not (the control law), rather than
reporting an aggregate transfer benefit.
...
even when training is entirely in simulation. Third, the pre-registered, audit-
...
transfer testcases under a pre-registered, audit-anchored protocol. The answer is
