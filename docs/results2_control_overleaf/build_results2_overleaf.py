...
def table_hypotheses() -> str:
    rows = [
        ...
        ("H2", "Calibrated v3.5 is useful when its role changes from dynamics provider to a frozen reward-shaping censor.",
         "\\S5", "SUPPORTED (hybrid gives best cross-window robustness)"),
...
This ablation resolves the timestep confound: improving v3's temporal resolution and predictive RMSE does not improve downstream utility---it destroys it, so v3's useful property is the optimization-friendly smoothing from temporal coarse-graining, not its black-box class alone. The pattern across the three single-model backends is near-monotonic in fidelity (hourly v3 usable; more accurate matched-v3 and v3.5 both fail), which reframes the hybrid as an explicit separation of smoothing (v3 rollout) from fidelity (frozen v3.5 censor): PPO needs not only accurate predictions but an optimization-friendly training landscape.

\paragraph{{Seed robustness.}} Retrained on three fixed seeds (\{{42,43,44\}}), each backend keeps its regime: pure v3 and the hybrid hold every seed below the $5\%$ violation bar on both windows, while the matched-resolution v3 collapses on every seed ($m_s=1.14$/$1.21$, std $<0.001$), so the usable-versus-collapse verdicts are seed-stable (Supplementary Table~\ref{{tab:seed_band}}, Fig.~\ref{{fig:seed_band}}); the single-seed peak edge of pure v3 over the hybrid does not persist across seeds (HDRL single-seed; MORL over $N{{=}}5$).
...
\begin{{figure}}[H]
  ...
  \caption{{Across $N=3$ seeds the fidelity--utility paradox is not a seed artefact: the usable hourly-v3 and hybrid controllers stay below both the $m_s=1$ closed-loop-collapse line and the $5\%$ comfort-violation reference on every seed, while the matched-resolution v3 collapses on every seed ($m_s>1$, $>85\%$ violation) with negligible seed variance. Markers are the mean over seeds \{{42,43,44\}} (filled = peak-heat window, open = typical-heat window); whiskers are $\pm$ one standard deviation; colour encodes the controller (green = coarse v3, red = matched-resolution v3, blue = hybrid). \textbf{{(left)}} live maintenance score $m_s$ (lower is better, with the usable $m_s<0.1$ band shaded); \textbf{{(right)}} comfort-violation fraction. Same data as Table~\ref{{tab:seed_band}}.}}
  \label{{fig:seed_band}}
\end{{figure}}

\paragraph{{Measured mechanism: response-surface smoothness.}} The smoothing is directly measurable on the surrogates, independent of any controller: sweeping each backend's one-step map $\hat T(a_0)$ over a grid of states, the scale-free \emph{{relative roughness}} (mean curvature $\div$ mean slope) orders cleanly with usability (Table~\ref{{tab:surface_sharpness}})---the usable hourly v3 is smoothest (${ctx['surf_v3_rel']}$), while both collapsing backends are several times rougher (${ctx['surf_matched_x']}\times$ matched-v3, ${ctx['surf_v35_x']}\times$ v3.5). This is the surrogate-side cause of the policy-side near-bang-bang action gap (Figs.~\ref{{fig:surface_curves}} and~\ref{{fig:action_phase}}); the raw slope/range are deliberately not used as the metric, being confounded by the step length.
...
\paragraph{{Engineering reading.}} As $\lambda_T$ rises, energy falls slightly but comfort collapses (peak violation roughly quadruples, $6.1\%\to23.0\%$): HDRL already supplies comfort-aware temporal abstraction, so the temperature censor is redundant and over-constrains the setpoint controller; the correct transfer keeps only the power channel ($\lambda_T=0$, $\lambda_P=5\times10^{{-5}}$).
...
\paragraph{{Engineering reading.}} The front is strongly asymmetric: energy-only (0/100) collapses to a non-heating policy ($86.8\%$ violation, $m_s={ctx['p0_ms']}$), comfort-only (100/0) gives the best single-seed $m_s$ at the highest energy, and the practical 75/25 point is the recommended compromise. The caveat is seeds: the $N=5$ extension turns both canonicals into wide distributions (CV ${ctx['n50_cv']}$--${ctx['n75_cv']}$), so the front is a best-seed envelope, not a deployment guarantee.
