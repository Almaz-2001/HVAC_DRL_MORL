...
- `fig_block2_hdrl_architecture` - schematic of the seasonal HDRL hierarchy
  (seasonal gate + winter/summer PPO specialists) and the over-regularization note.
- `fig_block2_morl_pipeline` - schematic of the MORL four-stage pipeline
  (pretrain / ERAM / live finetune / yearly eval) on the power-only backend.
...
Layout / structure (matches Results I exactly): no `\maketitle`; the package is
one `\section` (Results II = Section 5) with subsections 5.1-5.12; the
nomenclature table follows the pipeline figure (not a floating top table); all
floats use `[H]` with `float`/`placeins` so figures stay in place; the section
ends with a dedicated "Results II conclusion". Long file paths were removed from
table captions (provenance lives in roadmap Section 11.1).
