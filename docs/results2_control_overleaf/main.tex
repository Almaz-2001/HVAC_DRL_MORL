...
\begin{figure}[pos={!ht}]
  ...
  \caption{The fidelity--utility paradox in one view: surrogate predictive fidelity (24\,h rollout RMSE, $x$) versus downstream control utility (live maintenance score $m_s$, $y$; marker = peak/typical mean, whisker = the two windows). Across the three single-model surrogates the relationship is \emph{inverted}---the \emph{more} accurate the twin, the \emph{worse} the controller: the least accurate hourly v3 (RMSE $1.557\,^\circ$C) is the only usable training environment ($m_s\approx0.08$), while the more accurate matched-resolution v3 ($0.876\,^\circ$C) and the most accurate calibrated v3.5 ($0.644\,^\circ$C) both collapse ($m_s>1$). The role-separating hybrid breaks the trade-off: it rolls out on v3 (so its environment fidelity equals v3's, hence plotted at $1.557\,^\circ$C) but the frozen v3.5 censor drives $m_s$ to the usable regime. Lower predictive RMSE does not imply lower $m_s$.}
...
\begin{figure}[H]
  ...
  \caption{Measured response-surface smoothness of the three single-model surrogate backends, probed directly on each surrogate's one-step action map (no controller, no BOPTEST). \textbf{(A)} Measured centred one-step response $\Delta\hat T(a_0)$ in $^\circ$C for each backend, swept over a grid of zone temperatures and the two windows' ambients (faint = one curve per state, bold = mean). Note the per-panel $^\circ$C scales: the usable hourly v3 produces a large, smooth, monotone response, whereas the more accurate matched-resolution v3 and calibrated v3.5 produce a small but visibly non-monotone one. \textbf{(B)} The corresponding scale-free relative roughness (mean curvature $\div$ mean slope), independent of response amplitude and step length (bars = canonical aggregate matching Table~\ref{tab:surface_sharpness}; whiskers = standard deviation of the per-state ratio across the grid): the collapsing backends are $7.9$--$9.4\times$ rougher than the usable v3. \textbf{(C)} The measured policy-side consequence on the live emulator---the fraction of closed-loop steps with a saturated action ($|a_0|>0.9$): the usable hourly v3 modulates ($24\%$) while both collapsing backends are driven to a near bang-bang law ($100\%$). \textbf{(D)} The two linked across controllers: a rougher surrogate response surface goes with a worse live maintenance score $m_s$ (dotted line: the $m_s=1$ failure level; the hybrid, which rolls out on v3, sits with v3 in the smooth/usable corner). This is the surrogate-side \emph{cause} whose policy-side \emph{effect} is the near-bang-bang action gap of Fig.~\ref{fig:action_phase}. Data: \texttt{reports/block2\_mechanism\_surface\_sharpness.csv}.}
