...
\begin{figure}[H]
  ...
  \caption{MORL observation-interface ablation: 5D failure $\rightarrow$ 17D recovery. Two 5D bars are shown to match the main-text MORL 5D/17D comparison table: the \emph{current-code 5D rerun} ($m_s=0.680$, the value reported in that table) and the \emph{originally frozen 5D artifact} ($m_s=1.046$), which is retained only as an audit artifact. Both 5D variants fail; only the 17D power-only observation ($m_s=0.099$) recovers a usable policy. The observation interface, not only the scalarized reward, determines whether MORL is viable.}
  \label{fig:morl5d17d}
\end{figure}
...
\paragraph{Engineering reading.} The front is strongly asymmetric: energy-only (0/100) collapses to a non-heating policy ($86.8\%$ violation, $m_s=1.588$), comfort-only (100/0) gives the best single-seed $m_s$ at the highest energy, and the practical 75/25 point is the recommended compromise. The caveat is seeds: the $N=5$ extension turns both canonicals into wide distributions (CV $0.42$--$0.61$), so the front is a best-seed envelope, not a deployment guarantee.
