...
Table~\ref{{tab:transfer_matrix}} is the compact evidence summary and contains two distinct stories. The surrogate component transfers strongly: full Stage A/B/C improves RMSE by $60.2$--$87.8\%$ on all three target testcases. The frozen controller does not transfer uniformly: the two residential hydronic cases fail the pre-specified $1.25\times$ PI threshold, while the commercial stretch case passes the threshold but pays a $+35.3\%$ energy penalty. This component-level boundary --- the identified physics transfers, the frozen control law does not --- is consistent with prior evidence that building heterogeneity requires local adaptation rather than zero-shot reuse, motivating heterogeneous transfer-learning and online fine-tuning for real-world DRL deployment~\citep{{Coraci2024HeteroTL,Coraci2025HiLo}}.
...
Block~3 establishes a component-level transferability boundary: the inverse-calibration component transfers strongly (Stage~A/B/C improves RMSE by $60$--$88\%$ and re-identifies $C_{{\mathrm{{zon}}}}$ at ${ctx['czon_mean']}\pm{ctx['czon_std']}\times$ the \texttt{{bestest\_air}} value), whereas the frozen controller does not transfer as a deployment-ready policy (residential cases save energy but fail comfort; the commercial stretch passes $m_s$ only at $+35.3\%$ energy). The boundary lies at the controller--adapter interface, not in surrogate recalibration.

The pre-specification discipline is the methodological contribution: hypotheses, predictions, and the $1.25\times$ threshold were frozen before the runs, so the two falsifications (frozen-controller FAIL on the stretch case; scale-dependent $C_{{\mathrm{{zon}}}}$) are genuine Popperian updates. Block~4 follows directly --- target-specific fine-tuning on the recalibrated surrogate, not a frozen source controller, is the falsifiable path to deployment-ready hydronic transfer.
