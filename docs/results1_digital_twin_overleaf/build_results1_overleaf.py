...
The v3 input vector is intentionally low-dimensional. It keeps only the thermal state, outdoor disturbance, cyclic time embedding, and direct-\Tsupply{{}} action coordinates required by the control problem. The action coordinate \(a_0\in[-1,1]\) is mapped to an 18--35~$^\circ$C supply-temperature command, and the power head is constrained through a non-negative output transform. These choices make v3 a control-oriented surrogate rather than a generic black-box forecaster.
...
Unlike the purely black-box v3 head, the v3.5 surrogate is built on a first-principles lumped-capacity zone balance with a single physical parameter, the zone thermal capacitance \(\Czon\),
\begin{{equation}}
...
Every figure, table, and inline number in this section is regenerated from versioned project artifacts by a single command,
\begin{{verbatim}}
python docs/results1_digital_twin_overleaf/build_results1_overleaf.py
\end{{verbatim}}
which reads the canonical reporting tables (\texttt{{reports/hou\_evins\_*.csv}}, \texttt{{reports/block1\_corpus\_matched\_comparison.*}}, \texttt{{reports/speed\_benchmark\_table.csv}}) and the calibration summaries under \texttt{{outputs/surrogate\_v35\_inverse\_*}}. All timings are single-CPU-thread (\(\texttt{{torch\_num\_threads}}=1\)). The project additionally maintains pre-registration audit anchors in \texttt{{roadmap.md}} so that the Block 1--3 result chain is replay-checkable.
