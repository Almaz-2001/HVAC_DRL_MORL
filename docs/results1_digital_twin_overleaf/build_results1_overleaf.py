...
The canonical v3.5 is built by two sequential preset runs. \texttt{{block1\_15min\_episodeaware}} does the temperature-side inverse identification --- Stage~A aligns telemetry and removes latency/bias, Stage~B identifies \(\Czon\) on excitation-rich windows ({ctx['stage_b_epochs']} epochs), Stage~C refines residual heads ({ctx['stage_c_epochs']} epochs) while preserving the physical parameter. The second preset, \texttt{{block1\_15min\_power\_head\_only}}, freezes \(\Czon\), skips Stage~B, and refines only the power residual head ({ctx['power_stage_c_epochs']} epochs) --- which is why its JSON reports \texttt{{stage\_b\_epochs\_ran=0}}.
...
Table~\ref{{tab:multi-horizon}} exposes the split between the two families. The hourly v3 has a \emph{{negative}} 24~h \(R^2\) --- worse than the historical mean over long horizons, and worse than a naive persistence forecast at 24~h ({ctx['v3_hourly_24']} vs {ctx['persistence_24h']}~$^\circ$C) --- yet it remains a usable control surrogate because PPO needs smooth local gradients, not long-horizon realism. Calibrated v3.5 instead keeps a positive 24~h \(R^2\), bounds its tail (24~h P95 {ctx['p95_24h']}~$^\circ$C), and beats persistence at every horizon, confirming v3 is a control surrogate rather than a forecaster.
...
\begin{{equation}}
  ...
  \label{{eq:decomp}}
\end{{equation}}
which holds because the matched-architecture path is additive (the intermediate point is the same v3 retrained on the 15-min corpus). Of the total {ctx['delta_total']}~$^\circ$C drop from legacy v3 to calibrated v3.5, the matched-corpus report attributes {ctx['delta_corpus']}~$^\circ$C ({ctx['corpus_share']}) to the corpus shift and {ctx['delta_cal']}~$^\circ$C ({ctx['cal_share']}) to Stage~A/B/C. (An alternative split through the raw v3.5 backbone gives {ctx['alt_corpus_share']} corpus / {ctx['alt_cal_share']} calibration; the two differ because corpus and architecture interact non-additively, and we quote the matched-architecture split since both endpoints are control-architecture surrogates.) Either way the calibration claim is positive but bounded.
...
The validation is scoped to the \emph{{temperature}} channel, the comfort-relevant quantity that meets engineering tolerance (24~h RMSE {ctx['v35_cal_24']}~$^\circ$C, P95 {ctx['p95_24h']}~$^\circ$C, near-zero bias NMBE {ctx['nmbe_t_cal']}~$^\circ$C). The instantaneous \emph{{power}} channel is reported but not claimed as fully calibrated: MAE {ctx['mae_power']}~W on a mean {ctx['mean_power']}~W (CV(RMSE) $\approx{ctx['cv_rmse_power']}$\%, NMBE {ctx['nmbe_power']}\%), exceeding the ASHRAE Guideline~14 thresholds --- as expected for an instantaneous 15-min power signal of a cycling system, since Guideline~14 targets hourly/monthly-aggregated energy. Because every downstream KPI is evaluated on the live simulator, this residual power error does not propagate; it is disclosed as a genuine limitation and motivates the later power-disagreement regularizer.
...
Block~1 carries four results forward: the compact v3 surrogate ({ctx['v3_total_params']} parameters, {ctx['v3_rows']} hourly transitions) is fast and smooth but its 24\,h rollout error is {ctx['v3_hourly_24']}~$^\circ$C; v3.5 Stage~A/B/C identifies a physically bounded $\Czon$ of {ctx['czon_final']}~$\times10^5$~J/K and cuts the error to {ctx['v35_cal_24']}~$^\circ$C; the matched-corpus control attributes {ctx['corpus_share']} of that gain to the 15-minute corpus and only {ctx['cal_share']} to calibration; and v3.5 remains {ctx['v35_speedup']}x faster than BOPTEST. Block~1 thus supports the digital-twin claim but not yet a controller-transfer claim, which Results~II tests under live BOPTEST.
