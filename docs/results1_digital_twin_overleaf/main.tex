...
The validation is scoped to the \emph{temperature} channel, the comfort-relevant quantity that meets engineering tolerance (24~h RMSE 0.644~$^\circ$C, P95 1.207~$^\circ$C, near-zero bias NMBE -0.117~$^\circ$C). The instantaneous \emph{power} channel is reported but not claimed as fully calibrated: MAE 482~W on a mean 1002~W (CV(RMSE) $\approx69$\%, NMBE -12.4\%), exceeding the ASHRAE Guideline~14 thresholds --- as expected for an instantaneous 15-min power signal of a cycling system, since Guideline~14 targets hourly/monthly-aggregated energy. Because every downstream KPI is evaluated on the live simulator, this residual power error does not propagate; it is disclosed as a genuine limitation and motivates the later power-disagreement regularizer.
...
Block~1 carries four results forward: the compact v3 surrogate (8,482 parameters, 51,200 hourly transitions) is fast and smooth but its 24\,h rollout error is 1.557~$^\circ$C; v3.5 Stage~A/B/C identifies a physically bounded $\Czon$ of 4.413~$\times10^5$~J/K and cuts the error to 0.644~$^\circ$C; the matched-corpus control attributes 74.6\% of that gain to the 15-minute corpus and only 25.4\% to calibration; and v3.5 remains 114.2x faster than BOPTEST. Block~1 thus supports the digital-twin claim but not yet a controller-transfer claim, which Results~II tests under live BOPTEST.
